...
def day_17_1(values: list):
    ...
    steps_in_dir = 0
    cumulative_loss = 0
    curr_point = (cumulative_loss, s_pos, (0, 0), steps_in_dir)

    queue = [curr_point]
    heapq.heapify(queue)
    # BFS to find best path:
    visited = set(curr_point[1:])
    while queue:
        curr_point = heapq.heappop(queue)
        if curr_point[1] == (len(values) - 1, len(values[0]) - 1):
            # because we've sorted the queue, this is always the shortest path!
            queue = []
        else:
            for next_point in day_17_stepper(curr_point, values):
                if next_point[1:] not in visited:
...
def day_17_2(values: list):
    ...
    while queue:
        ...
        curr_point = heapq.heappop(queue)
        if (curr_point[1] == (len(values) - 1, len(values[0]) - 1)) & (curr_point[3] >= 4):
            # because we've sorted the queue, this is always the shortest path!
            queue = []
        else:
            for next_point in day_17_stepper(curr_point, values):
                if next_point[1:] not in visited:
                    ...
                    if next_point[3] <= 10:
                        # if we haven't moved 4 points yet, continue in same direction:
                        if (curr_point[3] < 4) & (next_point[2] == curr_point[2]):
                            heapq.heappush(queue,next_point)
                            visited.add(next_point[1:])
